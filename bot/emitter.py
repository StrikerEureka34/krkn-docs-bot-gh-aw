...
# env-comment is the source, so it is not recorded: the field marks only a
# description the bot could not take from source, and one grep finds them.
# krknctl counts, on the krkn-hub page: there it is the other source, and the
# next run must know not to treat the borrow as curation.
